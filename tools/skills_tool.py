...
def _get_skills_dir() -> Path:
    """Return the skills directory, resolved dynamically for profile support."""
    return get_hermes_home() / "skills"

# Legacy constant for backward compatibility (deprecated — use _get_skills_dir()).
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    """
    # Use dynamic skills directory for profile support.
    # Fall back to external dirs from config.
...
def skill_view(
    ...
    try:
        ...
        _trusted_dirs = [_get_skills_dir().resolve()]
        try:
        ...
        try:
            rel_path = str(skill_md.relative_to(_get_skills_dir()))
        except ValueError:
